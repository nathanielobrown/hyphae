...
- **Store handle** — one open trace store, `src/hyphae/store/handle.py:Store`: what a page or `hp query` holds instead of a connection, and `rows` on it is the one way a package outside the store runs SQL
- **Repository** — one family of reads over the handle, a method per statement, the widths and keys in and the models and citation out: `store.sessions` (`src/hyphae/store/sessions.py:SessionRepository`) is the first; a page names the repository and never the SQL
...
- **Forbidden contract** — the second contract in that table: only `store` may import `duckdb`, so a package outside it reaches the database through the `Store` handle (`src/hyphae/store/handle.py`) or a writer the store owns; the same run holds the code to both
