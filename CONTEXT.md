...
- **Scope** — corpus or keyed: whether a statement reads one of the relations `store.analysis` builds from `--project` (`src/hyphae/store/analysis.py`), which is what decides the flags a query takes
...
Model-written descriptions beside the telemetry: `docs/enrichment.md`; the vocabularies and the row: `src/hyphae/models/enrichment.py`; the items: `src/hyphae/models/items.py`; the tables and the repository over them, `store.enrichment`: `src/hyphae/store/enrichment.py`, and the reads that build each item: `src/hyphae/store/items.py`.
...
- **Widths** — a surface's fixed sizes, one field per query parameter it binds; a read names the surface, `store/library.py:bind` fills the mapping off it, the footer quotes it
...
- **Detail spec** — the one declaration of a Detail: its name, the fetch that answers it whole and the route that serves it, and how it was written; a node's own are `src/hyphae/view/detail.py:DETAILS`, a pass's lines `src/hyphae/view/enrichment.py:LINES`
...
- **Forbidden contract** — one of the two other contracts in that table, each naming an import the layers alone would allow: only `store` may import `duckdb`, so a package outside it reaches the database through the `Store` handle (`src/hyphae/store/handle.py`) or a writer the store owns; and nothing above the store imports `extract`, so a session is read back through the store rather than parsed again; the same run holds the code to all three
...
- **Cut** — a string shortened to a width, carrying one character past it so the page can mark it (the `cut` SQL macro, `view/text/format.py:cut`); the count a limit left off is *dropped* (`store/library.py:dropped`), printed as "+N more"
