...
| `origin` | `user` | Who sent the prompt | `tests/fixtures/spine/`, CC 2.1.221 |
| `origin.kind` | `user` | The sender, as `human`, `coordinator` or `peer` on a queued command. A `user` record also writes `task-notification` (3,949 records) and `auto-continuation` (1) | `tests/fixtures/interjection/`, CC 2.1.220 — `human`, on a prompt and on a queued command |
