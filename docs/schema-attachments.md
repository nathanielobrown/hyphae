...
| `attachment.origin.kind` | `attachment` | The sender, as `human`, `coordinator` or `peer`. A `user` record also writes `task-notification` (3,949 records: 371 of them a mid-turn notice flagged `isMeta`) and `auto-continuation` (1) | `tests/fixtures/interjection/`, CC 2.1.220 — `human`, on a prompt and on a queued command |
