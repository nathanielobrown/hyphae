...
A sweep also archives each full page — its DOM and every resource it fetched. `mise run e2e-chromatic` sends those archives to Chromatic, which renders each one in its own browser and diffs it against the project's baseline, so a font stack that differs between macOS and Linux never decides a diff. It wants `CHROMATIC_PROJECT_TOKEN` in `.env` or the environment and refuses before reaching the network without one (`tests/e2e/chromatic-upload.sh`). `.github/workflows/e2e.yml` runs the sweep and the upload on a pull request and on `main`; it is one of the two workflows in the repo holding a secret (`tests/tools/test_workflows.py` names both), and a fork's pull request runs everything but the upload rather than failing on a token it was never given. On a pull request it snapshots the branch's own tip rather than GitHub's merge preview, so a baseline you accept stays accepted when the base moves under it.
