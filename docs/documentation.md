...
A generator exposes `generate()`, which returns the block's body with no trailing newline, and a `main()` that prints it. Everything the block needs to say goes in the generator, including any heading or fence. A fence comes from `text.fence` in `tools/text.py` and sits flush against the markers, because aigarden counts a marker as the blank line a fence needs.
