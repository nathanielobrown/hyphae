...
After opening the PR, watch CI until it finishes green. Two workflows answer: `.github/workflows/check.yml` runs `mise run check` on Linux, which may expose differences hidden by your machine, and `.github/workflows/e2e.yml` runs the browser tier and uploads its pages to Chromatic ([the UI development guide](ui-development.md)). A third, `.github/workflows/claude.yml`, runs only when a comment or review on the PR mentions `@claude`: the action reads the thread, answers in a comment, and pushes a fix to the branch, which the first two then check. Poll a workflow run that can terminate: get its run ID, then run `gh run view <id> --json status` until the status is `completed`. Don't poll for the absence of pending checks; that state may never arrive.
