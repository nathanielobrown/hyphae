...
Every direct import between the drawn children of `hyphae`, from the importer to what it imports, generated by `mise run cogs` from the same graph the contract reads. `cli` is left off because it imports everything, and so are the leaf modules on the contract's two bottom lines, which any package may import. `cogs-check` in `check` fails when this diagram lags the code.
