...
- For a turn, the interjections it heard while it ran, in the order its transcript holds them: who sent each (a person, another agent, or a task's notice), when, the first 400 characters as they were written, and a link to the line holding the rest. The section lists 8 and counts the rest as "+N more"; a turn that heard nothing shows no section. A copy a fork replayed is never listed, and a message recorded before the first prompt belongs to no turn, so only the thread's transcript shows it
...
- If an extract starts while a page request holds the store, it waits that read out and lands — a page holds the file for one request, not for as long as the viewer is open. It waits up to ten seconds, and past that it stops, naming the store and the process holding it
