...
src/hyphae/               Analyze AI coding agents from their telemetry
  ...
  export/                 The OTLP exporter: `SessionTrace` in, spans to a backend out
  enrich/                 The enrichment layer: what a model wrote about each run, turn and session in the store
  analyze/                The analysis layer: the production defaults and the runner that binds and cites a library query
  view/                   The trace viewer: a local web app serving every node of a session as its own page
  store/                  The trace store: the schema, the writers that own every table in the DuckDB file, the reader that rebuilds a session from its rows, the versioned SQL library and macros every query reads through, the page reads a viewer request runs and the repositories that answer those reads and `hp query` in models, and the `Store` handle that hands those repositories out — the only reads any other package makes, since the driver never leaves this package
  models/                 The types that cross package lines: the trace model every extractor builds and sink writes, the enrichment types a pass, the store and a page share — the vocabulary, the items it describes and the row it writes — and what a store read hands back: the citation beside every answer, a node's values, the rows each repository builds for the page that reads them, and the answer `hp query` prints whole
