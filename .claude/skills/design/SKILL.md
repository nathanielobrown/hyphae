...
Write it as a handoff `design-<topic>` ( @../../../docs/handoffs.md ). It is per-run scratch: the PR's fact sheet draws its design points and plan deviations from it, so the reviewer sees what the diff doesn't make obvious (`.claude/skills/pr/fact_sheet.md`).
