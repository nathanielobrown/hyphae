...
| 2 | [The store package](phase-2-store-package.md) | 2.1 writer and schema, 2.2 reader, 2.3 enrichment tables, 2.4 delivery ledger | Each on the previous | Only `store`, `view` and `analyze` import `duckdb`; `extract` and `export` no longer touch each other |
