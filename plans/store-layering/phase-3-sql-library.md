...
3. **PR 3.2b: the fetch helpers.** `git mv src/hyphae/view/store.py src/hyphae/store/pages.py`; rewrite `hyphae.view.store` to `hyphae.store.pages` everywhere (`rg 'hyphae\.view\.store|hyphae.view import store' src tests tools`). Retarget `test_layout` and the viewer red-run leaf as above. As in 3.1, the comments that name the old path follow in their own commit, and two of them are `.sql` comments — `view_sessions.sql` and `view_described_sessions.sql` (`rg 'view/store' src/hyphae/store/queries`) — so that commit's render diff is exactly those two query pages plus the `view_sessions` scenario. Verify: `mise run check`; `python -m tools.gen_imports` draws the six edges.
