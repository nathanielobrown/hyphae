...
`exhaustive` names a module no line claims, and that a package added to a forbidden
contract's sources is named for the module it imports. Each case rewrites the real contracts
rather than pinning a copy, so a layer, a source or a contract added later is still the one
under test.
"""
...
def ini(fields: Mapping[str, object]) -> str:
    """One contract's fields in import-linter's `.ini` dialect: a list is one item per line."""
    return "".join(
        f"{key} =\n" + "".join(f"    {item}\n" for item in value)
...
def run_contract(
    layers: list[str], tmp_path: Path, *, sources: Mapping[str, list[str]] | None = None
) -> subprocess.CompletedProcess[str]:
    """`lint-imports` over the real package, under every contract `pyproject.toml` declares:
    the layers contract rewritten to these layers, and each forbidden contract as written, or
    over the sources `sources` gives it under the contract's name.
    ...
    """
    widened = dict(sources or {})
    written = []
    for at, declared in enumerate(contracts()):
        fields = dict(declared)
        if fields["type"] == "layers":
            fields["layers"] = layers
        elif fields["name"] in widened:
            fields["source_modules"] = widened.pop(fields["name"])
        written.append(f"[importlinter:contract:{at}]\n{ini(fields)}\n")
    # A source list for a contract the table does not declare would be a case testing nothing.
    assert not widened, f"no contract is named {sorted(widened)}"
...
def test_the_forbidden_contract_names_the_store_when_it_is_a_source(tmp_path: Path) -> None:
    ...
    sources = contract(DRIVER)["source_modules"]
    assert "hyphae.store" not in sources, "the store is a source now; retarget this case"
    done = run_contract(contract_layers(), tmp_path, sources={DRIVER: [*sources, "hyphae.store"]})
...
def test_the_forbidden_contract_lists_every_layer_but_the_store() -> None:
    ...
    sources = {name.removeprefix("hyphae.") for name in contract(DRIVER)["source_modules"]}
