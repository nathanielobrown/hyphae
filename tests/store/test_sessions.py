...
def test_the_mycelia_rollup_inside_the_corpus_window(repository: SessionRepository) -> None:
    ...
    assert len(rollups.rows) == 5


def test_the_landing_page_says_how_many_projects_its_limit_left_off(
    repository: SessionRepository,
) -> None:
    """Bound below the corpus's five projects, the rollups carry the rows the limit dropped
    ...
    assert rollups.cut == 4
    assert rollups.rows[0].matched_rows == 5
