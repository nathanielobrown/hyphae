...
def test_names_lists_every_file_in_the_directory_and_nothing_else(
    ...
    shipped = manifest.names()
    assert shipped == sorted(path.stem for path in queries.QUERY_DIR.glob("*.sql"))
    assert all(queries.load(name) for name in shipped)
...
def test_a_size_the_library_declares_is_the_documented_one(name: str, value: int) -> None:
    ...
    assert getattr(queries, name) == value
