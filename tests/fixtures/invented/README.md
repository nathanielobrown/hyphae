...
| `invented-unknown-lead.jsonl` | an `isMeta` `user` record with an `origin` whose first line is no known lead, with the sentinel past the 60 characters the error quotes | all 1,100 recorded relayed messages open on one of five lead lines (scanned 2026-09-25). It and the four rows below are built on records in `interjection/`: the untagged notice on the run's first task notice, the rest on its coordinator's message |
