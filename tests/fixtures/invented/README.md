...
| `invented-unknown-lead.jsonl` | an `isMeta` `user` record with an `origin` whose first line is no known lead, with the sentinel past the 60 characters the error quotes | all 1,100 recorded relayed messages open on one of five lead lines (scanned 2026-09-25). It is built on the coordinator's record in `interjection/`, as are the three below |
